...
const ADMIN_EMAILS = (process.env.ADMIN_EMAILS_CSV || '').split(',').map(e => e.trim()).filter(Boolean)
const SLACK_WEBHOOK_URL = process.env.SLACK_WEBHOOK_URL

async function processBirthdayDigest() {
  console.log(`[${new Date().toISOString()}] Starting birthday digest processing...`)

  try {
    // Get birthday digest data
    const digestData = await getBirthdayDigest(ORG_TZ)
    console.log(`Found ${digestData.today.length} birthdays today and ${digestData.thisWeek.length} this week`)
...
export function scheduleBirthdayCron() {
  ...
  console.log(`Scheduling birthday digest to run daily at ${CRON_LOCAL_HOUR}:00 ${ORG_TZ}`)
