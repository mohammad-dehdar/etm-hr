...
export default async function DashboardPage() {
  ...
  const completedTests = user.testAttempts.filter(
    attempt => attempt.status === 'SCORED'
  );
  const inProgressTests = user.testAttempts.filter(
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">اعلان جدید</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>آخرین تست‌های شما و وضعیت آنها</CardDescription>
            </CardHeader>
            <CardContent>
            ...
            {upcomingBirthdays.length === 0 ? (
              <div className="text-center py-6">
                ...
                <p className="text-gray-500">
                  هیچ تولدي در روزهای آینده وجود ندارد
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {upcomingBirthdays.map((birthday, index) => (
                  <div
                    key={index}
                    className="flex items-center space-x-3 rtl:space-x-reverse p-3 border rounded-lg"
                  >
