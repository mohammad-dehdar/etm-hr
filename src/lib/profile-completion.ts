...
export function calculateProfileCompletion(profile: Profile | null, userEmail?: string): ProfileCompletionResult {
  if (!profile) {
    return {
      percentage: 0,
      completedFields: [],
  ...
  // Check skills
  if (profile.skills && Array.isArray(profile.skills) && (profile.skills as any).length > 0) {
  ...
  // Check socials
  if (profile.socials && typeof profile.socials === 'object' && Object.keys(profile.socials as object).length > 0) {
  ...
  // Check documents
  if (profile.documents && typeof profile.documents === 'object' && Object.keys(profile.documents as object).length > 0) {
  ...
  if (profile.documents && typeof profile.documents === 'object') {
    ...
    const hasProfilePhoto = docs.profilePhoto || (Array.isArray(docs) && docs.some((doc: any) => doc.type === 'photo'))
    if (hasProfilePhoto) {
...
export function getNextRecommendedFields(percentage: number, missingFields: string[]): string[] {
